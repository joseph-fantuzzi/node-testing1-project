...
describe("[Exercise 3] findLargestInteger", () => {
  test("[5] returns the largest number in an array of objects { integer: 2 }", () => {
    const input = [{ integer: 1 }, { interger: 2 }, { integer: 3 }];
    const expected = 3;
...
describe("[Exercise 6] Car", () => {
  ...
  test("[15] driving the car returns the updated odometer", () => {
    ...
    expect(focus.drive(10)).toBe(20);
    expect(focus.drive(10)).toBe(30);
  });
...
describe("[Exercise 7] isEvenNumberAsync", () => {
  test("[19] resolves true if passed an even number", async () => {
    const answer = await utils.isEvenNumberAsync(20);
    expect(answer).toBe(true);
  });
  test("[20] resolves false if passed an odd number", async () => {
    const answer = await utils.isEvenNumberAsync(21);
    expect(answer).toBe(false);
  });
  test("[21] rejects an error with the message 'number must be a number' if passed a non-number type", async () => {
    try {
      await utils.isEvenNumberAsync("hello");
    } catch (err) {
      expect(err).toMatch("number must be a number");
    }
  });
